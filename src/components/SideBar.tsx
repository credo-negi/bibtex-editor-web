...
const SIDEBAR_ANIMATION_DELAY = 250;
...
const SideBar = ({ styleXStyles, ...rest}: SideBarProps) => {
    ...
    const handleClickToChangeColorScheme = useCallback(() => {
        switch (colorScheme) {
            case "default":
    ...
    const handleClickToChangeContrast = useCallback(() => {
        switch (contrast) {
            case "default":
                setContrast("medium");
                break;
            ...
            >
                ...
                <section {...stylex.props(style.secondary__container)}>
                    <Button
                        ...
                        onClick={handleClickToChangeColorScheme}
                        shrinkDuration={SIDEBAR_ANIMATION_DURATION}
                    >
                        {colorScheme === "light" ? "ダークモードに変更" : (colorScheme === "dark" ? "システムのカラー設定に合わせる" : "ライトモードに変更")}
                    </Button>
                    <Button
                        size="medium"
                        theme="tertiary"
